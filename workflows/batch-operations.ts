...
import { createWorkflow, parallelNode, sequenceNode } from '@jshookmcp/extension-sdk';
...
export const batchXssTestWorkflow = createWorkflow('workflow.batch-xss-test.v1', 'Batch XSS Test')
  ...
  .inputSchema({
    ...
    properties: {
      ...
      parameter: {
        type: 'string',
        description: 'Parameter to test (e.g., "search", "q")',
      },
      payloads: {
        type: 'array',
        items: { type: 'string' },
        description: 'XSS payloads to test',
      },
      maxConcurrency: {
        type: 'number',
